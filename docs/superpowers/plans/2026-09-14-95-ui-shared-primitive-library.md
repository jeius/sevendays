...
- **No Radix anywhere this ticket introduces:** after generation, `packages/ui` must depend on `@base-ui/react` (exact spelling — hyphenated; the published npm package, stable at 1.8.0 — the raw registry item's content field shows the legacy underscore name `@base_ui/react` 1.0.0-alpha.3, but the CLI rewrites generated imports to the published package name; verified on execution 2026-09-14: an interim amendment to the underscore spelling was itself wrong and reverted), never `@radix-ui/*` or `radix-ui`. The pre-existing, unused `radix-ui@^1.6.7` dependency in `apps/landing/package.json` stays untouched (fenced out — #96/#101 cleanup, not infra).

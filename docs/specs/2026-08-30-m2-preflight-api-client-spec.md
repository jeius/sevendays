...
_Decided 2026-08-30 (grill-with-docs session). Amended 2026-09-04 (readiness audit: AppType export mechanism, Env migration finish + Zod-validated env, landed-pieces annotations). Authoritative decision record: **ADR-0006**. Roadmap tracking: **Milestone 2 pre-flight block** in `docs/plan.md`. Related: ADR-0004 (auth session sharing), ADR-0003 (per-workspace vitest configs)._
...
- **Type-sharing via Hono RPC** ("using RPC with larger applications" pattern): the API's route sub-apps stay chained, and the API exports its app type (`AppType`). The client package takes the API as a **devDependency and imports only its types** — erased at runtime, no runtime coupling, no cycle. The `AppType` export resolves via a **named types-only subpath export** in `apps/api/package.json` — `"./app": { "types": "./src/index.ts" }` — with `export type AppType = typeof app` in `apps/api/src/index.ts`; clients do `import type { AppType } from '@sevendays/api/app'`. This keeps the coupling explicit (the API stays a non-runtime-consumable package), and it survives the tsconfig-variant `include`-replacement pitfall (the build tsconfig must keep listing the files the export graph needs). Consequence: any API route change re-typechecks the client package (intended), making "routes stay chained" a standing API review rule.
- **API restructure as a precondition**: export `AppType`; finish the `Env` migration — `apps/api/src/index.ts` still reads the ambient global (`new Hono<{ Bindings: Env }>()` with no import), so the root app switches to importing `Env` from `src/env.ts` and `env.ts` itself becomes **Zod-validated** (a schema parses the runtime binding object, failing loudly on a missing/malformed `DATABASE_URL` instead of trusting the ambient declaration); adopt the convention that every non-2xx response is `c.json({ error, ... }, <explicit status>)` — never a bare not-found helper, which degrades RPC response inference to `unknown`. Sub-apps already use the explicit `ApiEnv` from `services/db.ts` (`ApiEnv.Bindings` points at `env.ts`'s `Env`).
- **Runtime validation**: success-response schemas and an `apiErrorSchema` (`{ error: string, details?: unknown }`) live in the shared types package (project rule: shapes are defined there, not per app). `apiErrorSchema` **already landed** in `packages/types/src/api-error.ts` with tests (pulled forward during M1.4 — do not re-create it); what remains is the client's use of it. The client parses success payloads against the former and error responses against the latter, throwing a typed `ApiClientError` (status + parsed details) on non-2xx.
...
- **Seam 1b (env parsing)**: the Zod-validated env schema (from the restructure) is tested directly — a missing or malformed `DATABASE_URL` fails parsing loudly, a valid binding parses to the typed `Env`.

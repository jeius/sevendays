...
| 2026-09-22 | — (direct docs commit) | `6aff692` | split | — (pick pending) | Post-M3 docs-alignment pass (2026-09-22 audit catch-up): progress.md rotated to M4 + the closed #21 bullet moved to What Exists + dist path fixes; architecture.md packages/ui bullet + frontend-dependency sentence, tech-stack.md shadcn line, AGENTS.md primitive count re-aligned to ADR-0017; PRD.md V1 Scope delivery-editions annotation. Classifier SPLIT — 4 v1-paths (`AGENTS.md`, `docs/{PRD,architecture,tech-stack}.md`) + `docs/progress.md` main-only. Content pass: the AGENTS/architecture/tech-stack hunks are design-system prose (client-safe — the `87fe9e5` ruling class; no edition tokens); the PRD annotation trips edition vocabulary (v1/v2, spec #76, ADR-0015) — content-dropped at apply. **Pick NOT yet executed** (session close-out): next session cherry-picks `-n`, drops the PRD hunk (content rule) + progress (DU), commits as a split with the provenance + `Split:` lines, runs the locks (local gates, audit exit 0, push, run green with `Deploy v1 (private)` success / `Deploy teaser (main)` skipped), and fills this row's v1 SHA |

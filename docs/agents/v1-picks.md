...
| 2026-09-16 | #110 | `8c89d63` | split | — (pending) | M3 ticket 04 — landing surfaces onto the system — **plus the #111 UI remediation riding the branch** (Treatment B chrome + mobile panel B live, F1 `@layer base` contrast fix, variant D composition dev-switchable via `/?variant=d` + the `/prototype-remediation` ruling surface, `popover` registry addition, owner photos + `sd.png` monogram, the scoped defect list: /branches rhythm, Badge nowrap clipping, ServiceTeaserItem focus ring, not-found polish; the AGENTS.md UI-skills line). Classifier SPLIT — 54 v1-paths + 18 main-only (docs/plan/progress, the superpowers plan file, graphify-out). Content pass: booking substance = every CTA/deep-link hunk — hero "Book now" + teaser "Book now" (`index.tsx`), `PackageCard` "Book now"/"Book this package", `ServiceCard` "Book now", `BranchCard` "Book at this branch" (the `tel:` affordance stays — v1's primary per the #60 ruling), chrome CTAs in `site-header`/`site-footer`/`mobile-nav` (apply Treatment B styling around v1's call-forward content — same pattern as #105's executed pick), and the **entire prototype/D surface drops whole** per the #91/#103 prototype-surface precedent: `home-variant-a..d`, `card-system`, `card-system-compare`, `chrome-variants`, `copy.ts`, `prototype-switcher`, `prototype-remediation.tsx`, `routeTree.gen.ts` (regenerate after drops — verified `v1` has NO prototype routes, NO `scripts/` dir at all → `content-pages.mjs` hunk drops too), `public/photos/*` + README (main-showcase stand-ins v1 doesn't serve). Clean picks: `badge.tsx` (nowrap/min-h clip fix), `styles.css` (F1 fix), `walk-in-badge`, `service-teaser-item` (focus ring; CTA-less per #105), `branch-card` tel: + surface, `package-card`/`service-card` surface hunks, `services.tsx`/`about.tsx`/`branches.tsx`/`packages/*` rhythm + polish, `db-state.mjs` (import order), `AGENTS.md` UI bullet (client-safe prose, 87fe9e5 ruling class). Executor note: if the prototype components drop, `popover.tsx` has no v1 consumer — keep or drop is audit-neutral, executor's call. **Pick pending** — size-gated at merge time (a #105-scale execution); no order gate, backlog otherwise empty |

...
| 2026-09-14 | — | `87fe9e5` | split (pending) | — | direct docs commit, no PR: M3 design-system spec + ADR-0017 + AGENTS.md UI-rule amendment + plan/progress. Classifier SPLIT — 2 v1-paths (`AGENTS.md`, `docs/adr/0017-*.md`) + 3 main-only docs. Content pass: the three AGENTS.md hunks are client-safe (design-system prose, no edition tokens); ADR-0017 trips edition naming ("no shared-composed middle tier in v1") — scrub to "for now" when applying. Pick not yet executed (it pushes + deploys `v1`; next triager runs it). The two 2026-09-12 `chore(graphify)` direct commits need no rows — all-main-only paths (graphify-out), nothing pickable |
| 2026-09-15 (backfill) | #91 | `fd94164` | split (pending) | — | missed at merge (2026-09-12) — no row existed; the 2026-09-14 session's close-out backfilled #86–#89 but not this. Classifier SPLIT — 10 v1-paths + 2 main-only. Content scan: two flags for the pick — tokens.css's PROTOTYPE comment "Dark mode is PARKED for v1" is design-version numbering, not edition vocabulary, but scrub on apply; the `/prototype-tokens` mock sidebar's "Appointments" string is display-only (not a booking affordance; audit tokens unaffected). Pick hazards: `prototype-tokens.tsx` is post-seed (v1 may lack it — create whole or drop the hunk); styles.css + lockfile are transformed/regenerable. PICK ORDER: first (main order), then 87fe9e5, then #102 |
| 2026-09-15 | #102 | `6f8f220` | split (pending) | — | M3 ticket 01 — packages/ui shared primitive library: components.json trio `base-rhea`/lucide/zinc, `button` on Base UI (`@base-ui/react`), `cn` package consolidation, check pipeline, tracer renders both apps, docs + graph. Classifier SPLIT — 20 v1-paths + 27 main-only. Content pass CLEAN on all 20 (only grep hits are lockfile integrity-hash substrings — noise). AGENTS.md hunk is design-system prose (client-safe, same ruling class as 87fe9e5's). Pick order: after #91 and 87fe9e5. Pick hazards: same prototype-tokens/styles.css/lockfile notes as #91 |

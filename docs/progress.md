...
- `apps/api`: Hono app — `/health` (real) plus the four `/api/v1` route sub-apps over service modules (real Postgres reads/writes since M1.4; see the M1.4 entry above). Owns `vitest.config.ts` (node env, explicit include — required on vitest 4, see ADR-0003), `tsconfig.build.json` (emits `dist/`), and `worker-configuration.d.ts` included in both tsconfigs so the global `Env` resolves in every compile path.
...
1. **Milestone 2 pre-flight (#1)** — shared API client (`@sevendays/api-client`), `export type AppType` on the API (the one piece of plan.md's restructure checkbox still open — sub-app chaining, explicit `Env`, and the JSON-error convention landed early via M1.4 + candidates A–D), `API_URL` env wiring, React Query SSR in landing + admin. Checklist lives in `docs/plan.md`; spec in `docs/specs/2026-08-30-m2-preflight-api-client-spec.md`.

...
1. **Milestone 2 pre-flight (#1)** — the API restructure piece (ADR-0006) is closed: sub-app chaining and the JSON-error convention landed early via M1.4 + candidates A–D, and the restructure closed the rest — explicit `Env` (Zod-validated, ambient global retired) and the `@sevendays/api/app` types-only subpath export. What remains of the block: the shared API client package (`@sevendays/api-client`, #22), then per-app wiring — `API_URL` env + React Query SSR in landing and admin (#23/#24). Checklist lives in `docs/plan.md`; spec in `docs/specs/2026-08-30-m2-preflight-api-client-spec.md`.

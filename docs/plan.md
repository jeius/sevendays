...
- [ ] API session verification (ADR-0004): verification-only BetterAuth instance (bearer plugin) over the shared tables + `requireSession` middleware (uniform 401 envelope); the admin server-fn seam extracts the httpOnly session cookie server-side and forwards `Authorization: Bearer` over the `API` service binding (ADR-0016) — the browser never holds the raw token, no cross-domain cookies
- [ ] Public appointments list closed: `GET /api/v1/appointments` session-gated (the M2 "until M4" stopgap ends; `GET /api/v1/appointments/:id` stays public by uuid-opacity for the confirmation read-back; `POST` stays public — guest booking); api integration tests prove 401/expiry/revocation through the real middleware

...
- [ ] `API_URL` wired as server-side env in both apps (`.env.local` in dev, Workers vars in prod; no fallback — a missing env fails loudly) _(2026-09-05: admin half landed via #24 — `API_URL` read in `apps/admin/src/lib/api.server.ts` with no fallback, loud-failure verified in dev and under built-worker wrangler dev; `.env.example` added; landing half is #23; tick at #25 close-out.)_
- [ ] Install `@tanstack/react-query` in `apps/landing` + `apps/admin` with SSR query integration (loader `ensureQueryData` + `useSuspenseQuery` patterns) _(2026-09-05: admin half landed via #24 — react-query ^5.102.8 + react-router-ssr-query ^1.167.2, per-request client in router context, loader `ensureQueryData` + `useSuspenseQuery` verified live; landing half is #23; tick at #25 close-out.)_

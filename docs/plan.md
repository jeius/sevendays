...
## Milestone 3 — UI/UX Design System (the design milestone — wayfinder map #55, closed 2026-09-14)

The design-system milestone seated in the former Booking Availability slot (owner deferral 2026-09-09; seating made real 2026-09-11 per the delivery-versions spec). Red-penciled 2026-09-14 to match its spec (`docs/specs/2026-09-14-ui-ux-design-system-spec.md`, GitHub issue #94; component distribution is ADR-0017) — the output of the UI/UX wayfinder map (#55, all eight tickets resolved). The milestone designs both variants of the shared landing surfaces: main's booking-present forms and the ruled v1 booking-off variants (#60 carries the mandate; direction in spec § The Booking-Off Direction of `docs/specs/2026-09-11-delivery-versions-spec.md`).

- [✅] `packages/ui` becomes the shared design-system library (ADR-0017): shadcn monorepo pattern — one `components.json` per workspace (same style/iconLibrary/baseColor across all three), `shadcn add` routing primitives into the package on the Base UI base, the `cn` package replacing hand-rolled helpers; AGENTS.md's UI rule amended to match
- [✅] Token swap: the validated logo-palette layer (prototype branch #92 @ `9100672` — round-trip oklch, fixed points petrol `#06708e` + ink `#0e131a`, four reference tones, ramp 50–900, states, muted text `#686969`) replaces the interim teal preset as both apps' theme; `--background` = wash-base; typefaces self-hosted (Figtree / Roboto Slab / Geist Mono — the render-blocking Google Fonts `@import` dies); the starter island palette + vocabulary removed (clearing the `--surface-strong` collision); no `--chart-*` tokens
- [✅] Landing atmosphere + chrome: ink-led header (dark `SiteHeader` variant, band-CTA focus-visible normalization) + new `SiteFooter` on all surfaces; petrol-wash body with white cards + cool hairlines, gray-light demoted to emphasis strips, deep petrol accent only; mobile hamburger + collapsible panel (`MobileNav`, proper disclosure semantics)
- [✅] Landing surfaces onto the system, both variants styled (main booking-present + the v1 booking-off pieces): home full-width two-CTA hero band (strips unchanged in order), `/services` closing strip + neutral "attach to your booking" rewrite, `/packages/:slug` one primary affordance per variant ("Book this package" → `/book?package=` / "Call us" → `/branches`), `/about` + home-blurb skeleton polish (copy slots stay placeholder-marked); `WalkInBadge` → thin wrapper over shared `badge`; transients token-fit only _(2026-09-16: ticket 04 (#98) — hero band + typed Links, neutral add-on copy + the closing call-or-visit strip, CTA-less index + the single "Book this package" detail affordance + system not-found, BranchCard `tel:` + token sweep, badge collapse (2 of 3 pills; the wizard's fenced to #99), /about rhythm; CDP 43/43 + `pnpm check` 35/35; the v1 pieces styled + documented, swapped at pick time)_
- [✅] Booking flow componentized onto the #58 vocabulary — `ChoiceCard`, `HourChipGrid`, `StepProgress`, `ConfirmationCard` extracted; `SummaryRail` / `RejectionCard` restyled — zero flow changes (`data-*` seams preserved verbatim) _(2026-09-20: ticket 05 (#99) — the four extracts + rail/rejection on the system, contact gate as `contactSchema` (predicate-identical, boolean-gate only), third pill collapsed onto `WalkInBadge`, white-card step surface; CDP 15/15 + 16/16 + 12/12 + mutating e2e 7/7 over the local compose stack; lib-seam 6/56 → 7/62 with the existing files untouched; `pnpm check` 35/35; zero script/test edits)_
...
- [✅] Verify: same URLs + untouched API calls + `data-*` seams intact; landing lib-seam tests + `pnpm check` green for touched packages; M2 CDP read-only regressions green; owner per-surface screenshots accepted on both variants; `/prototype-tokens` gallery deleted at close-out — (2026-09-20, #101: the two #101 rulings landed — variant D flip (Ruling A, with the one ruled content-pages edit) + serif inner-page register (Ruling B); CDP 15/16/12 + lib-seam 7/62 unchanged + `pnpm check` 35/35; contrast pairs re-measured (pinned values reproduced, muted-on-wash-b 4.59); owner screenshots accepted on both variants incl. mobile; prototype gallery + surfaces deleted; v1 backlog drained (#110 split executed, three skips))
...
**Exit criteria:** every shipped landing surface wears the decided brand system in both variants (owner screenshots accepted), admin's shell + IA scaffold exists for M4/M5 to build into, the starter theme is gone, and the verification bar above passes with URLs, API calls, and `data-*` seams unchanged.

## Milestone 4 — Admin Auth (v1-track)

**Split made real 2026-09-11 (delivery-versions spec, GitHub issue #76):** BetterAuth is v1-track; the appointments dashboard moved out of this milestone to v2 (ruling 6 of #62). Red-penciled 2026-09-22 to match its spec (`docs/specs/2026-09-22-m4-admin-auth-spec.md`, GitHub issue #117) — BetterAuth pinned `^1.7.5` (npm latest, v1.7 docs line), the four original boxes expanded to the decided shape (login method, owner-CLI provisioning, verification mechanics, workerd per-request instance posture, list-only appointments gating). Tickets #118–#122 (native blocking: #118 → #119 → {#120, #121} → #122).

- [✅] Foundation: `better-auth@^1.7.5` in `apps/admin` + `apps/api`; auth tables generated into `packages/db/src/schema/auth.ts` (`auth generate --config apps/admin/src/lib/auth.ts --adapter drizzle --dialect pg`) — user/session/account/verification + the admin plugin's fields — joining the barrel + relations; migration 0005 via the house `db:generate`/`db:migrate` flow; env examples extended _(2026-09-22: ticket #118 landed — deps pinned `^1.7.5` both apps (lockfile 1.7.5); the schema generated via the config-purity pair `apps/admin/src/lib/auth.ts` (factory) + `auth.config.ts` (CLI instance export — the 1.7.5 CLI reads an exported instance, not a factory, so the spec's `--config` points at the latter file now); migration 0005 applied — five tables live; env examples + the deploy-workflow wiring (BETTER_AUTH_URL var + admin DATABASE_URL sync) landed, legs green. The checkbox-3 live secret/var puts remain for #122 — that box stays unticked until then.)_
- [✅] Admin auth server: `src/lib/auth.ts` factory (email+password, `disableSignUp`, `minPasswordLength` 12, `admin()` + `tanstackStartCookies()` last, database-backed rate limiting) with **per-request instances over per-request db clients** (ADR-0011 — the drizzle adapter captures the db, and module scope breaks on workerd); routes mounted at `/api/auth/*` (TanStack Start `server.handlers` route); `GET /api/auth/ok` probe green _(2026-09-22: ticket #119 landed — the route is `apps/admin/src/routes/api/auth/$.ts` (server.handlers GET/POST → auth.handler; mounting spike-proven on react-start 1.168.56), built per request over a per-request db client like the new `getSession`/`ensureSession` server fns (`auth.functions.ts` — #120's gate consumes them); probe green in dev with the corrected literal — `/api/auth/ok` returns `{"ok":true}` in 1.7.5, not the spec's `{"status":"ok"}`.)_
- [ ] Env + secrets posture: `BETTER_AUTH_SECRET` on admin + api (teaser + v1 targets — one-time `wrangler secret put`s); `DATABASE_URL` secret + workflow sync on both admin Workers (the admin app talks to Postgres now — ADR-0007 topology); `BETTER_AUTH_URL` per target riding deploys as a `--var` (the `API_URL` mechanism); dev `.env.local`/`.dev.vars` _(2026-09-22 note via #119: the dev `.env.local`/`.dev.vars` values now exist locally — gitignored, uncommitted, one shared `BETTER_AUTH_SECRET`; only the live teaser/v1 puts remain, so the box stays unticked until #122.)_
- [✅] Staff provisioning: owner-run `create-staff` CLI (`auth create-admin`, pinned 1.7.5) — the only user-creation path and the v1 password-reset path; no self-serve sign-up, no email flows (owner-operated, matching the #75 handover model) _(2026-09-22: ticket #119 landed — `create-staff` wraps the pinned `auth@1.7.5 create-admin` (password interactive-only; existing-users confirmation on); `docs/staff-provisioning.md` is the in-repo create + reset runbook; the first staff user exists in the dev database, sign-in curl-verified with the session row visible in Postgres.)_
- [✅] Login UI + shell gate: `/login` on the design system (UI-bearing ticket — skill set named, owner-reacted variants per the #94 amendment); `_shell` `beforeLoad` session gate with redirect preservation; sign-out in the shell user card _(2026-09-22: ticket #120 landed — `/login` transcribes the owner-ruled hybrid (split ink band at `lg`+ / ink canvas below; Warm copy — variants rendered + reacted in-session per #94, prototype branch `prototype/120-login-variants`); the `_shell` gate preserves the aimed URL through the round-trip; sign-out is the user card's inline button (Option 2) revoking via BetterAuth's own endpoint; verified live with the gate matrix, sign-in, identity, unchanged stubs, and revocation — stub screens render exactly as M3 left them.)_
- [✅] API session verification (ADR-0004): verification-only BetterAuth instance (bearer plugin) over the shared tables + `requireSession` middleware (uniform 401 envelope); the admin server-fn seam extracts the httpOnly session cookie server-side and forwards `Authorization: Bearer` over the `API` service binding (ADR-0016) — the browser never holds the raw token, no cross-domain cookies _(2026-09-23: ticket #121 landed — `apps/api/src/services/auth.ts` (createVerificationAuth + requireSession, per-request over acquireDb's handle, bearer plugin, explicit secret; uniform 401 `{"error":"Authentication required."}`); BETTER_AUTH_SECRET optional in envSchema so a secretless Worker keeps its public endpoints — the middleware owns the loud 500; admin seam `getSessionScopedApiClient` exported for #122/M5; 7 scratch-app tests over real Postgres incl. expiry + signOut revocation + secretless-throw.)_
- [✅] Public appointments list closed: `GET /api/v1/appointments` session-gated (the M2 "until M4" stopgap ends; `GET /api/v1/appointments/:id` stays public by uuid-opacity for the confirmation read-back; `POST` stays public — guest booking); api integration tests prove 401/expiry/revocation through the real middleware _(2026-09-23: ticket #121 landed — requireSession mounts BEFORE the list's query validator; the single-get stays public by uuid-opacity (explicit test + unchanged suite) and POST stays public (unchanged guest suite); the M2 Known-Gap test inverted to its 401 assertion; api suite 93→103. The teaser's live 401/200/revocation leg + the secret puts are #122's.)_
- [ ] Verify: login against the real tables on the teaser, authenticated read through the gate, unauthenticated rejection on the protected list, sign-out revocation; v1 picks land (SPLIT expected on the appointments edits) + login verified on `sevendays-v1-admin`; docs rotation (progress/tech-stack/`AGENTS.md` auth-state flip) + v1-picks ledger rows

**Exit criteria:** staff can log in, sessions verify on the API, the public appointments list is closed, and sign-out revocation is proven — on main's teaser and on `v1`.
...
- Milestone 4's checkboxes were red-penciled 2026-09-22 per the M4 admin-auth spec (`docs/specs/2026-09-22-m4-admin-auth-spec.md`, GitHub issue #117) — BetterAuth pinned `^1.7.5` with the ADR-0004 topology made concrete (bearer-token verification over the shared tables, per-request auth instances per ADR-0011, owner-CLI staff provisioning, the appointments list gated while the single-get stays public by uuid-opacity). Original four-checkbox shape (2026-08-30): BetterAuth wired + tables migrated, `BETTER_AUTH_SECRET` set, api verifies sessions on mutating routes, verify login + rejection.
- Milestone 3's checkboxes were written 2026-09-14 per the UI/UX design-system spec (`docs/specs/2026-09-14-ui-ux-design-system-spec.md`, GitHub issue #94) — the output of the UI/UX wayfinder map (#55, closed 2026-09-14 with all eight tickets resolved). The component-distribution ruling is ADR-0017, which amends AGENTS.md's UI-ownership rule (shared primitives + semantic tokens in `packages/ui`; brand and page-specific components app-local). Booking Availability's deferral annotation stays preserved under v2; nothing renumbers.

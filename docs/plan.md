...
- [ ] Verify: same URLs + untouched API calls + `data-*` seams intact; landing lib-seam tests + `pnpm check` green for touched packages; M2 CDP read-only regressions green; owner per-surface screenshots accepted on both variants; `/prototype-tokens` gallery deleted at close-out

...
- **Resend** — transactional email for booking confirmations. Requires a `NOTICE_API_KEY` secret on `apps/api` once wired in (`wrangler secret put NOTICE_API_KEY`).
...
| `NOTICE_API_KEY` | `apps/api` | `wrangler secret put` |

...
function checkPgUrl(label, value, expectedPort) {
  ...
  console.log(
    `[ok] ${label}: present, postgres URL, port ${port}${portOk ? '' : ` (expected ${expectedPort})`}`
  );
  return portOk;
}
...
if (!existsSync(dbEnvPath)) {
  console.log(
    `[FAIL] ${dbEnvPath} does not exist — USER: paste the Session-mode pooler URL (port 5432) into this file as DATABASE_MIGRATE_URL=<url>. Never paste it into chat.`
  );
  failed = true;
} else {
  ...
  if (dbEnv.DATABASE_MIGRATE_URL && !ok) {
    console.log(
      '       NOTE: the direct host (db.<ref>.supabase.co) is IPv6-only and unreachable from this machine — use the SESSION-MODE POOLER URL (port 5432) from Project Settings → Database → Connection pooling.'
    );
  }
  if (!ok) failed = true;
}
...
if (!existsSync(devVarsPath)) {
  console.log(
    `[FAIL] ${devVarsPath} does not exist — USER: paste the transaction-pooled URL (port 6543) into this file as DATABASE_URL=<url>.`
  );
  failed = true;
} else {
...
for (const p of [dbEnvPath, devVarsPath]) {
  try {
    ...
    console.log(
      `[FAIL] NOT gitignored: ${path.relative(repoDir, p)} — add it to .gitignore before proceeding`
    );
    failed = true;
  }
}
...
console.log(
  failed
    ? 'GATE: BLOCKED — fix the [FAIL] lines above (user actions), then rerun.'
    : 'GATE: PASS — live-DB tasks may proceed.'
);

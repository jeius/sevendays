...
export const adminPrintSizes = new Hono<ApiEnv>()
  .get('/', async (c) => {
    return c.json(await listAdminPrintSizes(c.get('db')));
  })
  .get('/:id', validatedParam(z.object({ id: z.uuid() })), async (c) => {
    const { id } = c.req.valid('param');
    const row = await getAdminPrintSize(c.get('db'), id);
    if (!row) {
      return notFound(c, 'Print size not found.');
    }
    return c.json(row);
  })
  .post('/', validatedJson(createPrintSizeSchema), async (c) => {
    const result = await createAdminPrintSize(c.get('db'), c.req.valid('json'));
    if (!result.ok) {
  ...
  .put(
    ...
    validatedJson(updatePrintSizeSchema),
    async (c) => {
      const { id } = c.req.valid('param');
      const result = await updateAdminPrintSize(c.get('db'), id, c.req.valid('json'));
      if (!result.ok) {

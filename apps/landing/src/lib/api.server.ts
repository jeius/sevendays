...
// over *.workers.dev (error 1042), so on Workers the API fetch routes through
// the `API` service binding instead of the public URL. `cloudflare:workers`
// only resolves inside workerd — dev and vitest take the network path via
// API_URL unchanged. The specifier is built at runtime so bundlers never try
// to resolve it statically, and the binding's fetch rides the client's
// custom-fetch seam (CreateApiClientOptions.fetch — the toLoopbackFetch
// precedent).
